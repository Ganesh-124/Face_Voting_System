...
def register():
    if request.method == 'POST':
        ...
        if registered:
            ...
            password2=encrypt(email, password1)
            cursor.execute("INSERT INTO users (username, email,voter_id ,password) VALUES (?, ?,?,?)", (username, email,voter,password1))
            conn.commit()
            conn_primary = sqlite3.connect(PRIMARY_DATABASE)
            cursor_primary = conn_primary.cursor()
            cursor_primary.execute("INSERT INTO users (username, email, voter_id,password) VALUES (?, ?, ?,?)", (username1, email1, voter1,password2))
            conn_primary.commit()
            conn_primary.close()
...
def voting_form():
    ...
    if data:
        ...
        if request.method == 'POST':
            ...
            cursor.execute('SELECT *  FROM users WHERE voter_id=? and password=? ', (voter_id,password))
